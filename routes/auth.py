...
def register():
    ...
    try:
        db().execute(
          ...
          (email, generate_password_hash(pw), first, last, int(time.time()))
        )
        db().commit()
...
@require_auth
def me():
    row = db().execute("SELECT id,email,first_name,last_name,nessie_customer_id FROM users WHERE id=?", (g.uid,)).fetchone()
